(function () {
    ...
    api.registerListener((id, data) => {
        ...
        let imuAgeVal = row('Sample ages (DMP quat / DMP compass)');
        ...
        actionButton('Start', gyroCalButtonsRow, () => callAction('gyroCalControl', { action: 'start' }).catch((e) => alert('start failed: ' + e)));
        let gyroCalSaveBtn = actionButton('Save', gyroCalButtonsRow, () => {
            callAction('gyroCalControl', { action: 'save' }).then((res) => {
                if (res.status === 'error') alert('save failed: ' + res.message);
            }).catch((e) => alert('save failed: ' + e));
        });
        actionButton('Cancel', gyroCalButtonsRow, () => callAction('gyroCalControl', { action: 'cancel' }).catch((e) => alert('cancel failed: ' + e)));
        ...
        let perfFifoVal = row('FIFO frames drained / overflows');
        ...
        function describeDisagreement(json) {
            let candidates = [];
            if (json.dmpHeadingValid) candidates.push(json.dmpHeading);
        ...
        function poll() {
            fetch('/api/user/icm20948Task/data')
                ...
                .then((json) => {
                    ...
                    imuAgeVal.textContent = (json.dmpSampleAgeMs ?? -1) + ' ms / ' + (json.dmpCompassAgeMs ?? -1) + ' ms';
                    imuErrorsVal.textContent = 'sensor ' + (json.sensorErrorCount ?? 0) +
                        ', fifo ' + (json.fifoErrorCount ?? 0) +
                        ', drain-limit ' + (json.fifoDrainLimitCount ?? 0);
                    imuRecoveriesVal.textContent = 'fifo resets ' + (json.fifoResetCount ?? 0) +
                        ', dmp reinits ' + (json.dmpReinitCount ?? 0) +
                        ', sensor reinits ' + (json.sensorReinitCount ?? 0);
            ...
            callAction('gyroCalStatus').then((json) => {
                ...
                if (json.state === 'done') {
                    gyroCalResultVal.textContent = fmtXYZ(json.biasX, json.biasY, json.biasZ, 3);
            ...
            callAction('perfStatus').then((json) => {
                ...
                perfFifoVal.textContent = json.fifoFramesDrained + ' / ' + json.fifoOverflows +
                    ' (drain limits ' + (json.fifoDrainLimits ?? 0) + ', resets ' + (json.fifoResets ?? 0) +
                    ', dmp reinits ' + (json.dmpReinits ?? 0) +
                    ', sensor reinits ' + (json.sensorReinits ?? 0) +
                    ', sensor errors ' + (json.sensorErrors ?? 0) + ')';
